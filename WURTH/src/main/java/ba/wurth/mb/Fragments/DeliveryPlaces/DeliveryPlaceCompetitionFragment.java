...
public class DeliveryPlaceCompetitionFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.deliveryplace_competitions, container, false);
    }
    ...
    private class LongTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            try {
                Thread.sleep(300);
                mCursor = DL_Additional.Get_Competition_Items_Values(DeliveryPlaceID);
            ...
            try {
                if (mCursor != null){
                    ...
                    while (mCursor.moveToNext()) {
                        ...
                        ((TextView) v.findViewById(R.id.litTitle)).setText(mCursor.getString(mCursor.getColumnIndex("CompetitionName")));
                        ((TextView) v.findViewById(R.id.litSubTitle)).setText(mCursor.getString(mCursor.getColumnIndex("ItemName")));
                        ((TextView) v.findViewById(R.id.litTotal)).setText(CustomNumberFormat.GenerateFormatCurrency(mCursor.getDouble(mCursor.getColumnIndex("Value"))));
                        ...
                        if (mCursor.getString(mCursor.getColumnIndex("CompetitionName")).length() > 0) ((TextView) v.findViewById(R.id.litStatus)).setText(mCursor.getString(mCursor.getColumnIndex("CompetitionName")).substring(0,1));
